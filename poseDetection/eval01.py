...
numIter = 2 # number of measurement iterations
...
seqNames = ['caviar-walkByShop1cor-seq000-5xSubsample',
            'caviar-walkByShop1cor-seq001-5xSubsample',
            'caviar-walkByShop1cor-seq002-5xSubsample',
            'caviar-walkByShop1cor-seq003-5xSubsample']
...
execTimeRef = evalTools.inferFramesetBenchmark(modelBaseline, frameset, numIter=numIter, 
                                               cuda=useCuda, preprocessor=poseDetEvaluator.preprocessor)
#measure power
if measurePower:
    powerLogger = evalTools.inferFramesetPowerMeasurement(modelTest, frameset, 
                                                          ...
                                                          preprocessor=poseDetEvaluator.preprocessor)
...
for seqIdx, seqName in enumerate(seqNames):
    ...
    clsfRef =  evalTools.inferFrameset(modelBaseline, [frameset[-1]], cuda=useCuda, 
                                       preprocessor=poseDetEvaluator.preprocessor)
    if target is None:
        target = poseDetEvaluator.targetGenerator(frameset[-1])
        lossRef = 0
    else:
        #there is no real reference if there is not target...
        lossRef = poseDetEvaluator.evaluator(clsfRef, target)
...
#        result.accuracy = clsf.eq(target).sum() / target.numel()
        result.loss = poseDetEvaluator.evaluator(clsf, target)
    ...
    if i == len(seqNames)-1:
        getFullEvalNumOps = reduce(lambda u,v: u+v, 
                                   map(lambda u: u['totalInputValues'], 
                                       resultsByThreshold[0].compStats))
...
if exportFiles:
    ...
    import csv
    import os
    filename = 'results/poseDet-%s-eval01-%s-%s.csv' % (
    ...
    evalTools.writeTable([header]+tbl, appName='poseDet', seqName=seqName, evalName='eval02b', 
